...
- локальный стенд для каталога предметов, карточек, поиска, фильтров и демо‑листингов.
...
Локальный каталог предметов для витрины. Сейчас используется как источник данных для интерфейса.
...
### 1. Пересобрать локальный каталог предметов
...
- обновляет цены в локальном каталоге.
